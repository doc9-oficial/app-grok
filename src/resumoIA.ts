...
export default async function resumoIA(params: any): Promise<void> {
  try {
    ...
    const response = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'grok-1',
        messages: [
          { role: 'system', content: 'Resuma o texto a seguir.' },
          { role: 'user', content: texto }
        ]
      })
    });
    const data = await response.json() as any;
    return console.log(docgo.result(true, { resumo: data.choices?.[0]?.message?.content || '', apiResponse: data }));
  } catch (error: any) {
